...
import sys
"""
OpenMayaMPx는 Maya API에서 플러그인을 확장하는 데 사용하는 클래스와 함수들이 포함된 모듈입니다. 
Maya Python API 1.0에서 사용되며, 사용자 정의 노드, 명령어, 도형, 속성, 그리고 툴을 만들기 위해
 제공되는 인터페이스입니다. 이 모듈을 통해 사용자 정의 플러그인을 작성하고 Maya에서 사용할 수 있도록 
 등록할 수 있습니다.

Maya Python API 1.0의 핵심 중 하나인 OpenMayaMPx는 주로 다음과 같은 상황에서 사용됩니다:

커스텀 노드(Custom Node) 정의: Maya에서 제공하는 기본 노드를 확장하여 새로운 기능을 가진 노드를 만드는 경우.
새로운 명령어(Custom Command) 정의: Maya의 명령어를 확장하거나 새로운 명령어를 만들 때.
새로운 도형(Custom Shape) 정의: Maya에 없는 커스텀 3D 도형을 추가할 때.
새로운 기즈모(Custom Manipulator) 정의: Maya의 조작 도구를 확장하여 커스텀 조작 도구를 구현할 때.
...
# 파이멜 코어는 뭘까요 그냥 파이썬 바인딩으로는 안되는 걸까
...
# 마야의 명령어 실행해주는 클래스인 것 같
    ...
    def doIt(self, argList):
        ...
        def recolor(re_obj, recol):
            object_check(re_obj) # 오브젝트가 존재하는지 확인하는 함수 (별도로 정의된 함수여야 함)
            crv = pm.PyNode(re_obj) # 오브젝트를 PyNode로 변환하여 Maya에서 제어 컬러노드 생성하는 것 같다
            crv.overrideEnabled.set(1) # Override 기능 활성화
            crv.overrideColor.set(recol) # 지정된 색상으로 변경
